...
contract('MultiSigWallet', (accounts) => {
    ...
    beforeEach(async () => {
      valorToken = await ValorToken.new(accounts[0], accounts[1], accounts[2]);
      multisigInstance = await MultiSigWallet.new([accounts[0], accounts[1], accounts[2]], 2);
      assert.ok(multisigInstance)
      assert.ok(valorToken);

      await valorToken.transfer(multisigInstance.address, 1000000, {from: accounts[0]});
    })

    it('Transfers SmartValor tokens after 2 confirmations', async () => {
        // Encode transfer call for the multisig
        const transferEncoded = valorToken.contract.transfer.getData(accounts[5], 1000000)
        ...
        assert.equal(1000000, await valorToken.balanceOf.call(accounts[5]))
    })
    ...
    it('fails when trying to approve a pending transaction by not an owner', async () => {
        ...
        assert.equal(1000000, await valorToken.balanceOf.call(tokenHolder))
    })
    ...
    it(' fails transfer execution if the multisig wallet does not have enought balance', async () => {
      ...
      const transferEncoded = valorToken.contract.transfer.getData(accounts[8], smartValorAmount)
      ...
      assert.equal(0, await valorToken.balanceOf.call(accounts[8]))
      ...
      assert.equal(smartValorAmount, await valorToken.balanceOf.call(accounts[8]));
    })
})
